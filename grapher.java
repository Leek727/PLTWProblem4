...
public class grapher extends JPanel {
     ...
     public void draw(Graphics g) {
          ...
          System.out.println(
                    "Max value: " + Math.round(max * 100) / 100 + "\nMin value: " + Math.round(min * 100) / 100);
     ...
     public static void main(String[] args) {
          ...
          if (graph.func.equals("")) {
               String coolFunctions[] = { "Math.cos(x)*Math.sin(y)", "x*x-y*y", "x*x+y*y",
                         "Math.sin(2 * (x*x + y*y))", "x*y*y*y-y*x*x*x", "-1/(x*x+y*y)"};
               String coolFuncNames[] = { "waves", "saddle", "paraboloid", "ripple", "spin???","Gabriel's Horn"};
               int coolIndex = new Random().nextInt(coolFunctions.length);
               graph.func = coolFunctions[coolIndex];
               System.out.println("\n\n\nChosen function: " + graph.func + "\n" + "Function name: "
                         + coolFuncNames[coolIndex] + "\n");
          }
